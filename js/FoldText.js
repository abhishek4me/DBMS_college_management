  ...
  const FoldText = ({
    ...
    useEffect(() => {
      ...
      const killTimeline = () => {
        ...
        if (window.gsap) {
          window.gsap.killTweensOf(pieces);
        }
      };
    ...
    return h(
      ...
      h('span', { className: 'fold-text-sr-only' }, text),
      h('span', { className: 'fold-text-visual', 'aria-hidden': 'true' }, segments)
    );
  };
