...
const { useEffect, useRef, createElement: h } = window.React || {
  useEffect: (cb) => cb(),
...
export default function Aurora(props = {}) {
  const {
    colorStops = ['#5c0d2e', '#2e0854', '#0d1330'],
    amplitude = 0.9,
    blend = 0.6,
    speed = 0.18,
    lightMode = false
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL2 Aurora initialization failed:", e);
      return;
    }
    ...
    const update = t => {
      ...
      const curAmp = propsRef.current.amplitude ?? amplitude;
      const curBlend = propsRef.current.blend ?? blend;
      const curLight = propsRef.current.lightMode ?? lightMode;
      const stops = propsRef.current.colorStops ?? colorStops;

      program.uniforms.uTime.value = (t * 0.01) * curSpeed * 0.1;
      program.uniforms.uAmplitude.value = curAmp;
      program.uniforms.uBlend.value = curBlend;
      program.uniforms.uLightMode.value = curLight ? 1 : 0;
...
function mountAurora() {
  const mountNode = document.getElementById('aurora-root');
  if (mountNode && window.ReactDOM) {
